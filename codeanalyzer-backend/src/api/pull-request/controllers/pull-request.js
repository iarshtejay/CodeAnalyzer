  ...
  ({ strapi }) => ({
    ...
    async getAvgTimeDifferenceBetweenPullRequests(ctx, _next) {
      ...
      for (let i = 0; i < prs.length - 1; i++) {
        let difference =
          (new Date(prs[i].createdOn).getTime() -
            new Date(prs[i + 1].createdOn).getTime()) /
          (1000 * 60 * 60 * 24);
    ...
    async getUsers(ctx, _next) {
      ...
      try {
        const contributors = await strapi.db
          .query("api::pull-request.pull-request")
          .findMany({
            select: ["username"],
            where: {
    ...
    async getPullRequestsCountsByBranch(ctx, _next) {
      const repository = ctx.request.query["repositoryId"];
      try {
        let result = [];
        const branches = await strapi.db
          .query("api::pull-request.pull-request")
          .findMany({
            select: ["targetBranch"],
            where: {
